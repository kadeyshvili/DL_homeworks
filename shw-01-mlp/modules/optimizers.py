...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m in zip(parameters, gradients, self.state['m']):
            ...
            """
            grad = np.add(grad, self.weight_decay * param, out=grad)
            m = np.multiply(self.momentum, m, out=m)
            m = np.add(grad, m, out=m)
            param = np.subtract(param, self.lr * m, out=param)
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m, v in zip(parameters, gradients, self.state['m'], self.state['v']):
            ...
            # lr - etta
            grad = np.add(grad, self.weight_decay * param, out=grad)
            m = np.add(self.beta1 * m, (1 - self.beta1) * grad, out=m)
            v = np.add(self.beta2 * v, (1 - self.beta2) * np.square(grad), out=v)
            m_hat = m / (1 - self.beta1 ** t)
            v_hat = v / (1 - self.beta2 ** t)
            param = np.subtract(param, self.lr * m_hat / (np.sqrt(v_hat) + self.eps), out=param)
